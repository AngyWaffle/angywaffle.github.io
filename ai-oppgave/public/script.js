document.getElementById('ftpForm').addEventListener('submit', function() {    
    ...
    .then(data => {
        ...
        if (data!== undefined) {
            ...
            alert(data.message); // Show a simple alert if connection failed
        }
    })
    .catch((error) => {
...
function updateFileList(path) {
    ...
    fetch('/navigate', {
        ...
        // Assuming authentication and other details are managed server-side
    ...
    .then(data => {
        if (!data.files) {
            ...
            return; // Exit the function or handle the missing data case
        }
        ...
        // Create a new list based on the fetched data
        ...
        //attachEventListeners(); // Reattach event listeners after updating the list 
    })
    .catch(error => {
...
function uploadThing() {
    ...
    // Assuming '/currentPath' holds the path of the currently open directory
    const currentPath = document.getElementById('currentPath').value; // Make sure this is set correctly
    ...
    .then(data => {
        if (data.success) {
            ...
            // Optionally refresh the file list to show the new file(s)
        } else {
